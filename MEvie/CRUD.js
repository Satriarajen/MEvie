...
export default function App() {
  ...
  useEffect(() => {
    ...
    // UTAK ATIK YG BAGIAN INI!!!!
    db.transaction(tx => {
      tx.executeSql('CREATE TABLE IF NOT EXISTS names (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT,tahun NUMERIC)')
    });
...
// BAGIAN INI JUGA DIUTAK-ATIK
  const addName = () => {
    db.transaction(tx => {
      tx.executeSql('INSERT INTO names (name, tahun) values (?, ?)', [currentName], [currentTahun],
      // '(tahun) values (?)', [currentTahun],
        (txObj, resultSet) => {
          let existingNames = [...names];
          existingNames.push({ id: resultSet.insertId, name: currentName, tahun: currentTahun});
    ...
    return names.map((name, index) => {
      return (
        <View key={index} style={styles.row}>
          <Text>{name.name}</Text>
          <Button title='Hapus' onPress={() => deleteName(name.id)} />
          <Button title='Perbaharui' onPress={() => updateName(name.id)} />
        </View>
      );
    });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Tambah Name" onPress={addName} />
      {showNames()}
